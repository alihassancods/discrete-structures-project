...
function parseSet(str) {
  ...
  function evaluatePredicate(domain, expr) {
    ...
    try {
      for(const x of domain){
        ...
        Function('x', '"use strict"; return (' + expr + ')')(x); // Evaluate the expression for side effects
      }
      return true;
    } catch(e) {
      return null;
  ...
  const observer = new MutationObserver(mutations => {
    mutations.forEach(mutation => {
      if (mutation.type === 'attributes' && mutation.attributeName === 'aria-labelledby') {
        if (contentSection.getAttribute('aria-labelledby') === 'prop-logic-title') {
        ...
        else if (contentSection.getAttribute('aria-labelledby') === 'set-ops-title') {
        ...
        else if (contentSection.getAttribute('aria-labelledby') === 'pred-logic-title') {
        ...
        else if (contentSection.getAttribute('aria-labelledby') === 'relations-title') {
  ...
  observer.observe(contentSection, { attributes: true });
  buttons.forEach(button => {
      button.addEventListener('click', () => {
          // Ensure event listeners are re-attached when content changes
          contentSection.innerHTML = ''; // Clear content before updating
          ...
          contentSection.setAttribute('aria-labelledby', button.getAttribute('data-target'));
